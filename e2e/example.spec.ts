...
test.afterEach(async ({ page }, testInfo) => {
  if (testInfo.status !== testInfo.expectedStatus) {
    console.log(
      `${testInfo.title} -did not run as expected, ended up at ${page.url()}`,
    );
  }
  page.close();
});
...
test("verificam search resultul", async ({ page }) => {
  ...
  const followers = page.locator(
    ".is-grouped > div:nth-of-type(3) .is-primary",
  );
  const followersCount = page.locator(
    ".is-grouped > div:nth-of-type(3) .is-info",
  );
...
test("Verify new dropdown", async ({ page }) => {
  ...
  const dropdownMenu = await page.locator(
    "select#fruits>option:nth-of-type(0)",
  );
...
test("Select date from calendar widget", async ({ page }) => {
  ...
  await page
    .locator(".is-danger.datetimepicker > .datetimepicker-container")
    .isVisible();

  // Click the desired day in the calendar
  await page.click(
    `xpath = //div[@class='datetimepicker is-danger is-active']//button[.='19']`,
  ); // Example for selecting day 18

  // Optionally, verify that the correct date has been selected
  const dateValue = await page.locator(
    ".card-content[_ngcontent-serverapp-c60] > .has-text-centered > [_ngcontent-serverapp-c60]",
