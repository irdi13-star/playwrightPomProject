  ...
  () => {
    test("Login/Logout successfully", async ({ app }) => {
      ...
      await test.step('Verify the successfully page', async () => {
        await app.common.headingIsVisible(labels.successfullyLoggedInPage.headerTitle);
        await app.common.paragraphIsVisible(labels.successfullyLoggedInPage.descriptionLabel);
        await app.common.linkedButtonIsVisible(labels.successfullyLoggedInPage.logoutButton);
      });

      await test.step('Verify log out action', async () => {
        await app.common.clickLinkedNamedButton(labels.successfullyLoggedInPage.logoutButton);
        await app.common.headingIsVisible(labels.loginPage.headerTitle)
      });
    });
    ...
    test("Incorrect password", async ({ app }) => {
      await test.step('Navigate to login page and fill form with invalid credentials', async () => {
        await app.base.navigateTo(routes.loginPageTests)
        await app.login.completeFormWithCustomData(credential.customerDetails.username_default, credential.customerDetails.invalid_passwrod);
      });
